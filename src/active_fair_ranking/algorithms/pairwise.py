...
class pairwise:
    ...
    def generate_deterministic_BTL_custom(self, w):
        self.w = w
        self.P = zeros((self.n, self.n))
        for i in range(self.n):
            for j in range(i, self.n):
                self.P[i, j] = w[i] / (w[i] + w[j])
                self.P[j, i] = 1 - self.P[i, j]
    ...
    def top1H(self):
        sc = self.scores()
        return 1 / (sc[0] - sc[1]) ** 2 + sum(
            [1 / (sc[0] - sc[1]) ** 2 for i in range(1, self.n)]
        )

    def top1parH(self):
        sc = self.scores()
        w = sorted(self.w, reverse=True)
        return ((exp(w[0]) - exp(w[1])) / (exp(w[0]) + exp(w[1]))) ** -2 + sum(
            [
                ((exp(w[0]) - exp(w[i])) / (exp(w[0]) + exp(w[i]))) ** -2
